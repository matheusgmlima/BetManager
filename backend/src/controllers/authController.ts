...
export async function verifyEmail(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!token) throw new AppError('Token obrigatório', 400, 'MISSING_TOKEN')
...
export async function login(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!email || !password) {
      throw new AppError('email e password são obrigatórios', 400, 'MISSING_FIELDS')
    }
...
export async function updateUnit(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (typeof unitValue !== 'number' || unitValue <= 0) {
      throw new AppError('unitValue deve ser um número positivo', 400, 'INVALID_UNIT_VALUE', 'unitValue')
    }
